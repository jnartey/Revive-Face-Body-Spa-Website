	...
	<div class="row">
		...
		<div class="large-10 columns content">
			<?php
				...
				echo $this->Form->create('Image', array('url' => array('controller' => 'images', 'action' => 'delete_selected', 'admin'=>true), 'id' => 'images-yard', array('type' => 'file')));
			?>
			<div class="row">
				<div class="large-12 columns">
					...
					<?php //echo $this->Html->link(__(("&lsaquo; back to products"), true), array('controller'=> 'products', 'action'=>'index', 'admin' => true), array('escape' => false, 'class'=>'button small primary')); ?>
					...
					<input type="submit" id="del-selected" class="button small alert" value="Delete Selected" />
					<?php
						echo '<label class="selec">'.$this->Form->checkbox('selectAll', array('value' => 1)).' Select All</label>';
					?>
				</div>
			</div>
			<div class="row">
				<div class="large-12 columns">
					<?php if($gallery): ?>
						...
						<div class="large-2 columns thumbs left">
							<div class="th radius text-center" href="#">
							<span class="top-buttons">
							<input type="checkbox" name="image_id[]" value="<?php echo $image['Image']['id'] ?>" />&nbsp;
							<?php
...
$(function() {
	...
	//$('#del-selected .counter').text(' ');
	$('#del-selected').val($('#del-selected').val() + ' ');
	
	var generallen = $("input[name='image_id[]']:checked").length;
	if(generallen>0){
		//$("#del-selected .counter").text('('+generallen+')');
		$('#del-selected').attr("value", 'Delete Selected (' + generallen + ')');
	}else{$("#del-selected .counter").text(' ');
		//$('#del-selected').val($('#del-selected').val() + ' ');
		$('#del-selected').attr("value", 'Delete Selected');
	}
})

function updateCounter() {
    var len = $("input[name='image_id[]']:checked").length;
	if(len>0){
		//$("#del-selected .counter").text('('+len+')');
		$('#del-selected').attr("value", 'Delete Selected (' + len + ')');
	}else{
		//$("#del-selected .counter").text(' ');
		$('#del-selected').attr("value", 'Delete Selected');
	}
...
$(document).ready(function() {
	$('#ImageSelectAll').on('change', function() {
		var checkthis = $(this);
		var checkboxes = $("input[name='image_id[]']");
